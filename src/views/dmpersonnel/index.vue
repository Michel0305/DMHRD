<template>
  <div class="personnel-container">
        ...
        <div class="grid-content bg-purple-dark">
          <el-row>
            <el-col :span="1" :offset="1">
              <el-button type="primary" icon="el-icon-edit" size="mini">入职</el-button>
            </el-col>
    ...
    <el-row>
      <el-col :span="24" >
        <el-table :data="tableData.filter(data => (!swhere ||
            data.user_name.toLowerCase().includes(swhere.toLowerCase()))
           || (!swhere || data.user_id.includes(swhere.toLowerCase())) )" 
        :max-height="contentStyleObj.height" style="width:100%">
          ...
          <el-table-column label="状态" sortable prop="tag" width="80">
            <template slot-scope="scope">
              ...
              disable-transitions>{{scope.row.isline?'在职':'离职' }}</el-tag>
            </template>
          </el-table-column>
...
　　　　　　height:'',
  ...
  methods: {
    handleEdit(index, row) {//显示
      this.rowData = row;
      this.showstatus = true;
    },

    cdialogFormStatus(val){//接收子主件消息
      console.log(val)
      this.showstatus = false
    },
    handleDelete(index, row) {},

    getHeight(){ //此处代码需要优化 全局使用
